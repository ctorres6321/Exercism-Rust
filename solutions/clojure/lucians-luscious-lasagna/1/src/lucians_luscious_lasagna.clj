...
(defn total-time
  ...
  [num-layers actual-time]
    ;; We can just bind this to the value inside the statement and return the actual value cleanly in our last statement 
  (def time-from-layers (* num-layers 2)) 
  (
   + time-from-layers actual-time 
  )
  )
